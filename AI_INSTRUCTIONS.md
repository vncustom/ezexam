...
- **Model dự phòng**: Tự động chuyển đổi nếu model hiện tại gặp lỗi/quá tải:
  1. `gemini-3.1-pro-preview` (ưu tiên 1)
  2. `gemini-flash-latest` (ưu tiên 2)
  3. `gemini-flash-lite-latest` (cuối cùng)
- **Cơ chế Retry**:
...
- **Giao diện**:
  - **Thiết lập Model & API Key**: Cần hiển thị như hình mẫu.
    ...
    - Thứ tự hiển thị: `gemini-flash-latest` (Default), `gemini-3.1-pro-preview`, `gemini-flash-lite-latest`.
